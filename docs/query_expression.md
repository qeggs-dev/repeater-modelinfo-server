...
1. `all`: 返回所有模型
2. `<provider>/<model_id>`: 返回该供应商下的指定模型，通常这种写法只能返回一个模型，可用于锁定模型请求
2. `<provider>`: 返回该供应商下的所有模型
4. `<model_id>`: 返回所有供应商下的该模型
5. `match:<regex>`: 返回所有 `2` 格式下满足正则表达式的模型
6. `search:<regex>`: 返回所有 `2` 格式下满足正则表达式的模型，但是包含该模式而非全字匹配该模式
